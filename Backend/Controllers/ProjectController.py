...
async def update_project(project_id: str, project_update: ProjectUpdateModel) -> dict:
    try:
        ...
        # Convert update model to MongoDB format
    ...
    try:
        ...
        for doc in staff_data:
            try:
                ...
                print(f"Error processing document {doc.get('_id', 'unknown')}: {doc_error}")
                continue
...
async def remove_project_from_staff(
    staff_id: str,
    ...
    try:
        ...
        result = await staff_collection.update_one(
            {"_id": staff_oid},
            {"$pull": {"assigned_projects": project_oid}}
        )
        ...
        if updated_staff:
            ...
            updated_staff["assigned_projects"] = [str(pid) for pid in updated_staff.get("assigned_projects", [])]
