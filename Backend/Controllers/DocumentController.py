...
SERVICE_ACCOUNT = (
    Path(__file__).resolve().parent / ".." / "Config" / "file-service.json"
)
...
async def addDocument(
    file: UploadFile = File(...), doc_name: str = Form(...), proj_id: str = Form(...)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get document ID: {str(e)}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Google Drive upload failed: {str(e)}"
        )
...
async def updateDocument(
    ...
    try:
        ...
        if update_result.modified_count == 0:
            raise HTTPException(
                status_code=500, detail="Failed to update document in database"
            )
        ...
        if drive_update_body:
            try:
                drive_service.files().update(
                    fileId=document["google_drive_id"], body=drive_update_body
                ).execute()
                ...
                raise HTTPException(
                    status_code=500, detail="Failed to update document in Google Drive"
                )
...
async def replaceDocument(
    docid: str, file: UploadFile = File(...), update_name: Optional[bool] = Form(False)
):
    try:
        # Convert docid to integer
        document_id = int(docid)
        ...
        drive_response = (
            drive_service.files()
            .update(fileId=existing_doc["google_drive_id"], **drive_update_body)
            .execute()
        )
        ...
        update_data = {
            ...
            "last_modified_date": drive_response.get(
...
async def deleteDocument(docid: str):
    ...
    try:
        ...
        # Try to delete from Google Drive, but ignore "file not found" errors
        try:
            drive_service.files().delete(fileId=document["google_drive_id"]).execute()
        except Exception as drive_error:
            # If the error is "File not found", continue to delete from MongoDB
            if "File not found" in str(drive_error):
                pass  # File already deleted, continue
            else:
                raise HTTPException(
                    status_code=500,
        ...
        # Delete from MongoDB
        delete_result = await document_collection.delete_one(
            {"document_id": document_id}
        )

        if delete_result.deleted_count == 0:
            raise HTTPException(
                status_code=500, detail="Failed to delete document record from database"
            )
...
async def fetchRecents():
    try:
        ...
        if not recent_docs:
            return JSONResponse(
                {"status": "success", "message": "No recent documents found"}
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch recent documents: {str(e)}"
        )
...
async def getDocsfromProject(proj_id: str):
    try:
        ...
        project_docs = await cursor.to_list(length=100)  # <-- FIXED
